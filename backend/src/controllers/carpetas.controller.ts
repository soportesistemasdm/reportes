...
// Estructura estricta del nodo del árbol
...
export const obtenerArbolAutorizado = async (req: Request, res: Response) => {
  // 1. Extraemos el rol_id de manera segura según la nueva estructura del request
  const userRolId = (req as any).usuario?.rol_id; 
  ...
  try {
    ...
    const mapeo: { [key: number]: FolderNode } = {};
