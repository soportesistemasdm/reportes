...
export const obtenerCarpetasPorRol = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    const idsCarpetas = resultado.rows.map((row: { carpeta_id: number }) => row.carpeta_id);
...
export const crearRolConPermisos = async (req: Request, res: Response) => {
...
export const actualizarRolConPermisos = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { nombre, carpetas } = req.body;
