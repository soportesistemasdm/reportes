...
import { verificarToken } from './authMiddleware.js'; // 👈 Importamos el middleware
import './scheduler.js';
...
import { obtenerUsuarios, crearUsuario, actualizarUsuario } from './controllers/usuarios.controllers.js';
dotenv.config();
const app = express();
...
app.get('/api/reportes/descargar/:id', verificarToken, async (req, res) => {
    try {
        const { id } = req.params;
        const permisosUsuario = req.usuario?.permisos || [];
